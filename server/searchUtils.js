...
async function searchRequest(query) {
  const endpoint = `https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=album&market=US&limit=5`;
  ...
  for (const album of albums) {
    try {
      ...
      const hasExplicit = albumRes.data.tracks.items.some((track) => track.explicit);
...
async function fullSearchRequest(query) {
  const endpoint = `https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=album&market=US`;
  const headers = await tokenManager.getAuthHeader();
  ...
  for (const album of albums) {
    try {
      const albumRes = await axios.get(`https://api.spotify.com/v1/albums/${album.id}`, { headers });
      const hasExplicit = albumRes.data.tracks.items.some((track) => track.explicit);
      album.isExplicit = hasExplicit;
    } catch (err) {
      console.error(`Error checking album ${album.id}:`, err.message);
...
async function getSimilarArtists(artistId) {
  ...
  for (const artist of res.data.similarartists.artist) {
    try {
      ...
      console.warn(`Couldn't find ID for artist: ${artist.name}`);
    }
  }
