...
export async function getSimilarArtists(artistId) {
  // Step 1: Get artist name from ID
  const artist = await getArtistInfo(artistId); // this calls Spotify API
  ...
  for (const artist of res.data.similarartists.artist) {
    try {
      const id = await getArtistId(artist.name); // fetch Spotify ID for each
