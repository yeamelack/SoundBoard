...
app.get("/artistProfile", async (req, res) => {
  const query = req.query.q; // artistId
  try {
    const artistInfo = await getArtistInfo(query); // renamed to avoid conflict
    // console.log(artistInfo);
...
app.get("/relatedArtists", async (req, res) => {
  const artistId = req.query.q;
  console.log("🔍 Incoming request to /relatedArtists for artistId:", artistId);
  try {
    const artists = await getSimilarArtists(artistId);
    console.log("✅ Similar artists fetched:", artists.length);
    ...
    console.error("Related artists fetch failed:", error.response?.data || error.message);
    res.status(500).send("Internal server error");
  }
});